...
public class OrganizerAppController {
    ...
    @Value("${pubsub.vote.topic-id}")
    private String voteTopicId;

    @Value("${pubsub.vote.subscription-id}")
    private String voteSubscriptionId;
    ...
    public ResponseEntity<String> receiveVote() {
        ProjectSubscriptionName subscriptionName = ProjectSubscriptionName.of(projectId, voteSubscriptionId);
        var wrapper = new Object() { String messageFromPubSub; };
        MessageReceiver receiver = (PubsubMessage message, AckReplyConsumer consumer) -> {
            System.out.println("Message Id : " + message.getMessageId());
            wrapper.messageFromPubSub = message.getData().toStringUtf8();
